...
- ค่าคงที่ทั้งหมดอยู่ใน `constants.ts` ไฟล์เดียว ห้าม hardcode ตัวเลขในโค้ดอื่น
  - ข้อยกเว้นเดียว: ตัวเลขแปลงหน่วยเวลา (`60_000` ms ต่อนาที, `1_000` ms ต่อวินาที) เขียนตรงๆ ได้ เพราะเป็นนิยามของหน่วย ไม่ใช่ค่ากติกาที่จะมีวันปรับ — `constants.ts` เองก็ใช้ `60_000` ตรงๆ ในนิยามของตัวมัน
